...
if __name__ == "__main__":
    model_path = r"C:\Users\SAPTARSHI MONDAL\PycharmProjects\CarbonModel\Model\track_fault_detection.pt"
    input_path = r"C:\Users\SAPTARSHI MONDAL\PycharmProjects\CarbonModel\Video\original.jpg"
    ...
    # Fake GPS start position (Kolkata)
    gps_lat, gps_lon = 22.5726, 88.3639
    gps_step_m = 0.00005  # ~5 meters per frame

    # ===== Load Model =====
    print("Loading track fault detection model...")
    ...
    if os.path.isfile(input_path) and input_path.lower().endswith((".mp4", ".avi", ".mov")):
        ...
        out_video = cv2.VideoWriter("output_track_fault.mp4",
                                    cv2.VideoWriter_fourcc(*"mp4v"), 20,
                                    (int(cap.get(3)), int(cap.get(4))))
        is_video = True
    else:
        image_files = [input_path] if input_path.lower().endswith((".jpg", ".png", ".jpeg")) \
            else sorted(Path(input_path).glob("*.*"))
        is_video = False
    ...
    while True:
        ...
        draw_boxes(draw_frame, detections, color=(0, 165, 255), label_prefix="FAULT: ")
    ...
    # Save CSV log
    pd.DataFrame(alerts).to_csv("alerts_track_fault.csv", index=False)
    print("\n✅ Track fault detection finished — Output: alerts_track_fault.csv")
